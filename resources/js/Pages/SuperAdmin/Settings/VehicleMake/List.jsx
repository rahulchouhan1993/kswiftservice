...
export default function List({ list, search }) {
    ...
    return (
        <AuthenticatedLayout>
            <Head title="Manage Vehicle Makes " />
                                        ...
                                        >
                                            ...
                                            <td className="p-1 flex justify-center items-baseline gap-4">
                                                <div data-tooltip-target={`tooltip-edit-vehicle-make-${l?.uuid}`}>
                                                    <Edit make={l} />
                                                    <Tooltip
                                                        targetEl={`tooltip-edit-vehicle-make-${l?.uuid}`}
                                                ...
                                                <div data-tooltip-target={`tooltip-status-${l?.uuid}`} className="flex">
                                                    <StatusToggle
                                                        action={route(
                                                            "superadmin.settings.vehicle.make.update.status",
                                                            { uuid: l?.uuid }
                                                        )}
                                                        checked={l?.status === 1}
                                                        className="!mb-0"
                                                ...
                                                <div data-tooltip-target={`tooltip-delete-vehicle-make-${l?.uuid}`}>
                                                    <DeleteUserAction
                                                        action={route(
                                                            "superadmin.settings.vehicle.make.delete",
                                                            { uuid: l?.uuid || "" }
                                                        )}
                                                ...
                                                <Tooltip
                                                    targetEl={`tooltip-delete-vehicle-make-${l?.uuid}`}
